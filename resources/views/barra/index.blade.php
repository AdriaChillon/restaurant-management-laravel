        ...
        <div class="flex justify-center">
            <div class="w-full lg:w-3/4">
                ...
                <div id="comandas-container">
                    ...
                    <div class="border-2 w-fit border-[#253080] shadow-xl rounded-lg p-6 mb-4 comanda" data-comanda-id="{{ $comanda->id }}">
                        <h2 class="text-lg font-semibold mb-2">Comanda #{{ $comanda->id }}</h2>
                            <p class="mb-2">Mesa: {{ $comanda->mesa->numero }}</p>
                            <p class="mb-2">Fecha y Hora:
                                {{ \Carbon\Carbon::parse($comanda->fecha_hora)->format('d/m/Y H:i') }}</p>
            ...
            return $producto->categoria->nombre === 'Refrescos' || $producto->categoria->nombre === 'Cafes';
                                            ...
                                            <li class="flex justify-between items-center mb-2">
                                                <div class="flex items-center">
                                                    ...
                                                    <span class="ml-4">x{{ $producto->pivot->cantidad }}
                                                        {{ $producto->nombre }} -
                                                        {{ ucfirst($producto->pivot->estado_preparacion) }}</span>
                                                </div>
                                            </li>
                                        @endforeach
                                    </ul>
                            ...
                            <div class="flex justify-between items-center mt-4">
                                <form class="form-cobrar" action="{{ route('barra.cobrar', $comanda->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="button"
                                        class="btn-cobrar bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Marcar
                                        como pagada</button>
                                </form>
                                <button type="submit"
                                    class="mt-6 px-4 py-2 bg-blue-500 hover:bg-blue-700 rounded-md text-white font-bold">Guardar
                                    Cambios</button>
                            </div>
                        </div>
                    @endforeach
